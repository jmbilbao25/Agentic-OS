...
# Done when
`https://<host>/healthz` returns 200 over real TLS; Google OAuth admits exactly one
account and rejects everything else; the orbit UI renders the live vault; a RAG answer
...
- [ ] create the Google OAuth client, put ID + secret + `OPENROUTER_API_KEY` in `server/.env`
- [ ] `tailscale up` then `tailscale funnel 8000` — confirm the public HTTPS hostname resolves
- [ ] set `OAUTH_REDIRECT_URI` to the funnel hostname, restart, complete one real Google login
- [ ] confirm a second Google account is rejected with 403 (this is the security test, not a formality)
...
- Security rule: the SG opens **22 only**. The app is never bound to a public interface
  directly — Funnel terminates TLS and forwards to localhost. An OAuth app with a
  single-email allowlist behind a plain-HTTP port is not protected, it is decorated.
