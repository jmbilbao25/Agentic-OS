...
ledger carries state across context resets. This is what you use on a hosted
surface — the agent cannot re-invoke itself, so *you* are the loop body and the
ledger is the loop counter. A new session picks up mid-loop with zero re-explaining.
...
Some harnesses ship a structured task system — a spec, a plan file, a
dependency-graphed task list with cross-session resume. Where one exists, prefer it
for **feature work with clear requirements up front**, and keep loop ledgers for
open-ended, exploratory, or maintenance work that doesn't decompose in advance.

Two rules when using a native system instead:

- The ledger still wins for anything spanning *harnesses* or machines. A vendor's
  task file is not portable; `brain/loops/` is.
- Never run both for the same work. Two sources of truth about what's done next is
