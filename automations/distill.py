...
def main():
    ...
    print("distill: reading %s (%d chars) via %s"
          % (src.name, len(text), ", ".join(llm.models())), file=sys.stderr)

    body = asyncio.run(run(text))
    if not body.strip() or body.startswith("[no model"):
        print("distill: model produced nothing usable:\n%s" % body[:400],
              file=sys.stderr)
        return 1
    ...
    doc = "\n".join([
        ...
        "model: %s" % srv_config.LLM_MODEL.split(",")[0],
        "---",
        "",
