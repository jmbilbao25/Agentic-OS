...
# --- auth: one user, one password ---
# The password is stored as a PBKDF2 hash, generated by:
#   python -m server.tools.setpass
# A plaintext AUTH_PASSWORD is accepted as a convenience but warned about, because
# .env gets copied, backed up, and pasted into chat windows.
...
# Brute-force throttle. A password on the public internet needs this; OAuth got
...
def problems():
    ...
    if not auth_configured() and not auth_bypassed():
        out.append("no password set — run: python -m server.tools.setpass")
    if AUTH_PASSWORD and not AUTH_PASSWORD_HASH:
        out.append("AUTH_PASSWORD is plaintext in .env — run "
                   "'python -m server.tools.setpass' to store a hash instead")
    ...
    if auth_configured() and BASE_URL and not BASE_URL.startswith("https"):
        out.append("AGENTOS_BASE_URL is not https — a password over plain HTTP is "
